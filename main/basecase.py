...
#=============================================自定义方法  BEGIN ==============================================================
	...
	def find(self,by,value,nocheck=False):
		if by not in ['id','accessibility_id','class_name','css_selector','name','link_text','partial_link_text','tag_name','xpath','ios_uiautomation','android_uiautomator']:
			raise ActionError("'find' function doesn't support such type:'%s'" %by)
		...
		try:
			...
			if not nocheck:
				raise ActionError("Element not found using '%s' : %s" %(by,value))
			else:
				return None
	...
	def finds(self,by,value,nocheck=False):
		...
		try:
			...
			if not nocheck:
				raise ActionError("Elements not found using '%s' : %s" %(by,value))
			else:
				return []

	def click(self,by,value,nocheck=False):
		if not isinstance(value, str):
			raise ActionError("'click' function required a str type on 'value' parameter")
	...
	def waitfor(self,by,value,desc="",timeout=10):
		try:
			...
			raise ActionError("'%s:%s' element not shown after %s seconds '%s'" %(by,value,timeout,desc))
